/**
 * @file CC1101.cpp
 * @brief Minimal CC1101 driver for TX‑only operation.
 *
 * This module provides:
 *   - Correct CC1101 reset sequence (TI SWRS061)
 *   - Register initialization for 433 MHz, 38.4 kbps 2‑FSK
 *   - Variable‑length packet mode (max payload = 61 bytes)
...
static const struct {
    ...
    // --- GDO Configuration ---------------------------------------------------
    {0x00, 0x06}, // IOCFG2: GDO2 = Assert on sync word, deassert end of packet
    {0x02, 0x06}, // IOCFG0: GDO0 = Assert on sync word
    ...
    // --- Main Radio Control --------------------------------------------------
    {0x18, 0x18}, // MCSM0: Auto‑calibrate on IDLE→TX
    ...
    // --- Front End -----------------------------------------------------------
    {0x21, 0x56}, // FREND1: Front‑end TX configuration
    {0x22, 0x10}, // FREND0: Front‑end TX power amplifier
...
// -----------------------------------------------------------------------------
// Strobe Commands (SRES, STX, SIDLE, SFTX, etc.)
// -----------------------------------------------------------------------------
uint8_t CC1101_Strobe(uint8_t strobe)
{
    uint8_t rx = 0;
    CC1101_Select();
    HAL_SPI_TransmitReceive(&hspi1, &strobe, &rx, 1, HAL_MAX_DELAY);
    CC1101_Deselect();
    return rx;
}
...
{
    if (len > 63) len = 63; // TX FIFO is 64 bytes total

    uint8_t buffer[64];
    buffer[0] = addr | 0x40; // Burst write header
...
{
    uint8_t header = addr | 0xC0; // Burst read header
...
// -----------------------------------------------------------------------------
// CC1101 Reset (TI‑specified sequence, SWRS061)
// -----------------------------------------------------------------------------
void CC1101_Reset()
{
    // CS high
...
void CC1101_Init()
{
    ...
    // Load modem + radio configuration
