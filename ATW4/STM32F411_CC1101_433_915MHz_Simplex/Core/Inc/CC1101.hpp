...
public:
	...
	// CC1101 Frequency Calculation (Integer-only, Compile-time)
	static constexpr uint32_t F_OSC = 26000000;
	static constexpr uint32_t F_CARRIER = 433000000;

	// The formula is: FREQ = (F_CARRIER * 2^16) / F_OSC
	// We use uint64_t for the intermediate to prevent overflow (433MHz * 65536 > 32-bit max)
	static constexpr uint32_t FREQ_VAL = static_cast<uint32_t>((static_cast<uint64_t>(F_CARRIER) << 16) / F_OSC);

	// Split into the 3 registers required by the CC1101
	static constexpr uint8_t FREQ2 = (uint8_t)((FREQ_VAL >> 16) & 0xFF);
	static constexpr uint8_t FREQ1 = (uint8_t)((FREQ_VAL >> 8) & 0xFF);
	static constexpr uint8_t FREQ0 = (uint8_t)((FREQ_VAL >> 0) & 0xFF);
...
static constexpr typename CC1101<GPIO>::RegisterValueTuple cc1101_init_regs[] = {
	...
	// F_CARRIER MHz center frequency (per TI calculator)
	{CC1101<GPIO>::ConfigurationRegister::FREQ2, FREQ2},
	{CC1101<GPIO>::ConfigurationRegister::FREQ1, FREQ1},
	{CC1101<GPIO>::ConfigurationRegister::FREQ0, FREQ0},
